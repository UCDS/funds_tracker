	...
	<div ng-controller ="Bank_bookCtrl as bank_bookCtrl">
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<div class="panel panel-yellow ">
				<div class="panel-heading">Bank Transactions</div>
					...
					<table class="table tableevenodd table-hover" style="font-size: 11px;">
						<thead>
							<tr>
								...
								<th title="Voucher Number">
									VchrNo
									<span class="sortorder" ng-show="bank_bookCtrl.propertyName === 'VchrNo'" ng-class="{reverse: bank_bookCtrl.reverse}"></span>
								</th>
						...
						<tbody>
							...
							<tr class="active">
								...
								<td ng-bind="bank_bookCtrl.MapBankName(bank_bookCtrl.newBank_book.bank_id)" style="display:none"></td>
								<td ng-bind="bank_bookCtrl.newBank_book.debit_credit"></td>
								<td class="num-align" ng-bind="bank_bookCtrl.newBank_book.transaction_amount | currency : '&#x20b9 ' "></td>
								...
								<td ></td>
								<td ></td>
								<td ng-bind="bank_bookCtrl.newBank_book.bill_recieved | mapYesNo" style="display:none;"></td>
								<td ng-bind="bank_bookCtrl.newBank_book.notes" style="display:none"></td>
								<td ></td>
							</tr>
							<?php
								}
							...
							<tr ng-repeat="bank_book in bank_bookCtrl.bank_books" > <!-- | orderBy:bank_bookCtrl.propertyName:bank_bookCtrl.reverse -->
								...
								<td class="num-align" nowrap ng-bind="bank_book.balance_ui | currency : '&#x20b9 ' "></td>
								<td class="num-align" nowrap ng-bind="bank_book.statement_balance_ui| currency : '&#x20b9 ' "></td>
								...
								?>
								<td>
									<span class="file-count-icon" ng-click="bank_bookCtrl.AttachFiles(bank_book)" title="Attachments">
										<i aria-hidden="true" class="fa fa-file-o"></i>
										<span class="file-count" ng-bind="bank_book.attachments_count"></span>
									</span>
									
									<span class="fa-buttons" title="Edit" ng-click="bank_bookCtrl.editRecord(bank_book,$index)"> <i class="fa fa-pencil-square-o fa-2" aria-hidden="true"></i> </span>
									<input type="button" ng-attr-value="{{bank_book.debit_credit == 'Debit' ? 'Payment Voucher' : 'Receipt Voucher'}}" ng-click="bank_bookCtrl.PrintVoucher(bank_book)"/>
...
//  			self.reverse = true;
			...
			self.PrintVoucher = function(bankbook){
				if(bankbook.debit_credit == "Debit")
					window.open("<?php echo base_url(); ?>bank_book/print_payment_voucher/"+bankbook.transaction_id, ""	);
				else
					window.open("<?php echo base_url(); ?>bank_book/print_receipt_voucher/"+bankbook.transaction_id, ""	);
			}
			self.JournalVoucher = function(bankbook){
				if(bankbook.debit_credit == "Debit")
					...
					window.open("<?php echo base_url(); ?>bank_book/print_receipt_journal_voucher/"+bankbook.transaction_id, ""	);
			}
			...
			self.editRecord = function(record,vIndex){
				...
				self.originalRecord = angular.copy(record); 
			...
			self.addTransaction = function() {
				...
				//alert(JSON.stringify(changedFields));
				if (changedFields.length > 0) 
				{
					...
					$http.post('bank_book/save_transaction_changes', JSON.stringify(changeLog), {
						headers: {
						...
						.then(function(response) {
							console.log('Changes logged successfully:', response);
						})
				...
				if(self.newBank_book.ledgerDetails && self.newBank_book.ledgerDetails.length > 0){
					...
					$.each(self.newBank_book.ledgerDetails,function(i,v)
					{
						...
						if (originalRecord) 
						{
							if (originalRecord.amount !== v.amount)
							{
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Amount",
									oldValue: originalRecord.amount,
									newValue: v.amount
								});
							}
							var oldLedgerAccountId = originalRecord ? originalRecord.ledger_account_id : null;
							if (originalRecord.ledger_account_id !== v.ledger_account_id) {
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Ledger Account",
									oldValue: oldLedgerAccountId === null ? '' : oldLedgerAccountId,
									newValue: v.ledger_account_id
								});
							}
							var oldLedgerSubAccountId = originalRecord ? originalRecord.ledger_sub_account_id : null;
							if (originalRecord.ledger_sub_account_id !== v.ledger_sub_account_id) {
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Ledger Sub Account",
									oldValue: oldLedgerSubAccountId === null ? '' : oldLedgerSubAccountId,
									newValue: v.ledger_sub_account_id
								});
							}
							var oldNarration = originalRecord ? originalRecord.narration : null;
							if (originalRecord.narration !== v.narration) {
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Narration",
									oldValue: oldNarration === null ? '' : oldNarration,
									newValue: v.narration
								});
							}
							var oldPayeePartyId = originalRecord ? originalRecord.payee_party_id : null;
							if (originalRecord.payee_party_id !== v.payee_party_id) {
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Payee Party",
									oldValue: oldPayeePartyId === null ? '' : oldPayeePartyId,
									newValue: v.payee_party_id
								});
							}
							var oldItemId = originalRecord ? originalRecord.item_id : null;
							if (originalRecord.item_id !== v.item_id) {
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Item",
									oldValue: oldItemId === null ? '' : oldItemId,
									newValue: v.item_id
								});
							}
							var oldProjectId = originalRecord ? originalRecord.project_id : null;
							if (originalRecord.project_id !== v.project_id) {
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Project",
									oldValue: oldProjectId === null ? '' : oldProjectId,
									newValue: v.project_id
								});
							}
							var oldDonorPartyId = originalRecord ? originalRecord.donor_party_id : null;
							if (originalRecord.donor_party_id !== v.donor_party_id) {
								changedFields.push({
									ledger_id: v.ledger_id,
									field: "Donor Party",
									oldValue: oldDonorPartyId === null ? '' : oldDonorPartyId,
									newValue: v.donor_party_id
								});
							}
						}
					});
				...
				$http.post('bank_book/add_transaction', tempDetails)
					.then(function(data){
						if(data){
							data = data.data;
							if(data){
								var ledgerDataForPost = [];
								$.each(self.newBank_book.ledgerDetails,function(i,v){
									v["transaction_id"] = data.TrnxID;
									v["debit_credit"] = data.debit_credit;
									v["ledger_reference_table"] = 1;
									ledgerDataForPost.push(v);
								});
								if(ledgerDataForPost.length > 0) {
									var req = {
											method: 'POST',
											url: 'ledger/addLedgerTransaction',
											headers: {
												'Content-Type': 'application/json'
											},
											data: JSON.stringify(ledgerDataForPost),
			...
			self.getLedgerTransactions = function(vTrnxID){
				$http.post('ledger/getLedgerTransactions/'+vTrnxID+"/1")
					.then(function(data){
						self.newBank_book.ledgerDetails = data.data;
						self.originalledgerRecord = angular.copy(data.data);
						//alert(JSON.stringify(self.originalledgerRecord));
					});
			}
			/**
			 * Mapping functions
