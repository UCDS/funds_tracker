...
<script>
  document.addEventListener('DOMContentLoaded', function() {
	...
	var viewInput = document.getElementById('view_value');
	var availableFilesDiv = document.querySelector('table.available_files');
    ...
    if (toggleCheckbox) {
      ...
      toggleCheckbox.addEventListener("change", function() {
		...
		if(viewInput.value === '1') {  
			availableFilesDiv.style.display = 'block';
		} else {
			availableFilesDiv.style.display = 'none';
		}

		if (togglebtn === 1 && availableFilesDiv) {
          availableFilesDiv.style.display = 'block';
        } else if (availableFilesDiv) {
          availableFilesDiv.style.display = 'none';
        }
			var transId= <?php echo $TranxID;?>;
		$.ajax({
			type: 'POST',
			data: {
				tranxID: transId,
			...
			url: '<?php echo base_url();?>upload/toggle_public_private',
			success:function(data, textStatus, jqXHR) 
            {
				if (togglebtn === 1) {
					alert('Private access enabled');
					window.location.reload();
				} else {
					alert('Public access enabled');
					window.location.reload();
				}
            },
...
<div ng-app ="uploadApp" class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
	...
	<div ng-controller ="LedgerReport as LedgerCtrl">
		...
		?>
			...
			<?php if($session_view[0]->view==1) { ?>
				<input type="hidden" name="view" id="view_value" value="1" />
			<?php }else { ?>
				<input type="hidden" name="view" id="view_value" value="<?=$view?>" />
			<?php } ?>
		...
		<form id="fileupload" action="" method="POST" enctype="multipart/form-data">
			...
			<div class="row" style="display:flex;">
				<div class="col-md-12">
					...
					<label style="font-size:20px;color:red;"> Enable Public / Private Access</label>
				</div>
			</div>
			<?php if($session_view[0]->view==0){ ?>
			<table role="presentation" class="table table-striped available_files" ><tbody class=""></tbody></table>
			<?php }else{ ?>
			<table role="presentation" class="table table-striped available_files" ><tbody class="files"></tbody></table>
			<?php } ?>
		</form>
		<script>
			window.onload = function() 
			{
				var viewValue = document.getElementById('view_value').value;
...
{% var fileCount = o.files.length; %}  <!-- Variable to store the total count of files -->
<h1>Files Count: {%=fileCount%}</h1>   <!-- Display the count -->
{% for (var i=0, file; file=o.files[i]; i++) { %}
